...
# Input
bulasik_miktar = np.arange(0,101,1)
kirlilik_derece = np.arange(0,101,1)
bulasik_cins = np.arange(0,101,1)

# Output
yikama_zaman = np.arange(30,161,1)
deterjan_miktar = np.arange(0,101,1)
su_sicaklik = np.arange(35,71,1)
ust_pompa = np.arange(2100, 3500, 1)
alt_pompa = np.arange(2100, 3500, 1)

# Input MF
bulasik_miktar_az = fuzz.trimf(bulasik_miktar, [0,0,35])
bulasik_miktar_ort = fuzz.trimf(bulasik_miktar, [15,50,85])
bulasik_miktar_cok = fuzz.trimf(bulasik_miktar, [65,100,100])

kirlilik_derece_az = fuzz.trimf(kirlilik_derece, [0,0,35])
kirlilik_derece_ort = fuzz.trimf(kirlilik_derece, [15,50,85])
kirlilik_derece_cok = fuzz.trimf(kirlilik_derece, [65,100,100])

bulasik_cins_has = fuzz.trimf(bulasik_cins, [0,0,35])
bulasik_cins_kar = fuzz.trimf(bulasik_cins, [15,50,85])
bulasik_cins_guc = fuzz.trimf(bulasik_cins, [65,100,100])
...
miktar_level_az  = fuzz.interp_membership(bulasik_miktar, bulasik_miktar_az, input_miktar)
miktar_level_ort = fuzz.interp_membership(bulasik_miktar, bulasik_miktar_ort, input_miktar)
miktar_level_cok = fuzz.interp_membership(bulasik_miktar, bulasik_miktar_cok, input_miktar)
kirlilik_level_az  = fuzz.interp_membership(kirlilik_derece, kirlilik_derece_az, input_kirlilik)
kirlilik_level_ort = fuzz.interp_membership(kirlilik_derece, kirlilik_derece_ort, input_kirlilik)
kirlilik_level_cok = fuzz.interp_membership(kirlilik_derece, kirlilik_derece_cok, input_kirlilik)
cins_level_has = fuzz.interp_membership(bulasik_cins, bulasik_cins_has, input_cins)
cins_level_kar = fuzz.interp_membership(bulasik_cins, bulasik_cins_kar, input_cins)
cins_level_guc = fuzz.interp_membership(bulasik_cins, bulasik_cins_guc, input_cins)
...
deterjan_activation_ca = np.fmin(rule1, deterjan_miktar_ca)
sicaklik_activation_dus = np.fmin(rule1, su_sicaklik_dus)
ust_pompa_activation_cd = np.fmin(rule1, ust_pompa_cd)
alt_pompa_activation_cd = np.fmin(rule1, alt_pompa_cd)
"""
...
zaman_activation_o = np.fmin(rule2, yikama_zaman_o)
deterjan_activation_n = np.fmin(rule2, deterjan_miktar_n)
sicaklik_activation_yuk = np.fmin(rule2, su_sicaklik_yuk)
ust_pompa_activation_d = np.fmin(rule2, ust_pompa_d)
alt_pompa_activation_cy = np.fmin(rule2, alt_pompa_cy)
"""
...
zaman_activation_o = np.fmin(rule3, yikama_zaman_o)
deterjan_activation_n = np.fmin(rule3, deterjan_miktar_n)
sicaklik_activation_nor = np.fmin(rule3, su_sicaklik_nor)
ust_pompa_activation_y = np.fmin(rule3, ust_pompa_y)
alt_pompa_activation_y = np.fmin(rule3, alt_pompa_y)
"""
...
zaman_activation_cu = np.fmin(rule4, yikama_zaman_cu)
deterjan_activation_cf = np.fmin(rule4, deterjan_miktar_cf)
sicaklik_activation_yuk = np.fmin(rule4, su_sicaklik_yuk)
ust_pompa_activation_d = np.fmin(rule4, ust_pompa_d)
alt_pompa_activation_cy = np.fmin(rule4, alt_pompa_cy)
"""
IF (miktar-cok & kirlilik-c & cins-guc) THEN zaman-u, deterjan-cf, sıcaklık-y, ust-pompa-cy, alt-pompa-cy
"""
rule5 = np.fmin(miktar_level_cok, np.fmin(kirlilik_level_cok, cins_level_guc))

"""
IF (miktar-cok & kirlilik-c & cins-has) THEN zaman-cu, deterjan-f, sıcaklık-y, ust-pompa-cd, alt-pompa-cd
"""
rule6 = np.fmin(miktar_level_cok, np.fmin(kirlilik_level_cok, cins_level_has))

"""
IF (miktar-orta & kirlilik-a ) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cy
"""
rule7 = np.fmin(miktar_level_ort, kirlilik_level_az)

"""
IF (miktar-orta & kirlilik-c & (cins-has | cins-kar)) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cy
"""
rule8 = np.fmin(miktar_level_ort, np.fmin(kirlilik_level_cok, np.fmax(cins_level_has, cins_level_kar)))

"""
IF (miktar-orta & kirlilik-c & cins-guc) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cy
"""
rule9 = np.fmin(miktar_level_ort, np.fmin(kirlilik_level_cok, cins_level_guc))

"""
IF (miktar-orta & kirlilik-o & (cins_has | cins-kar)) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cy
"""
rule10 = np.fmin(miktar_level_ort, np.fmin(kirlilik_level_ort, np.fmax(cins_level_has, cins_level_kar)))

"""
IF (miktar-cok & kirlilik-o ) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-o
"""
rule11 = np.fmin(miktar_level_cok, kirlilik_level_ort)
"""
IF (miktar-cok & kirlilik-a ) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cd
"""
rule12 = np.fmin(miktar_level_cok, kirlilik_level_az)
"""
IF (miktar-az & kirlilik-a & (cins_kar | cins-guc)) THEN zaman-cu, deterjan-cf, sıcaklık-y, ust-pompa-d, alt-pompa-cd
"""
rule13 = np.fmin(miktar_level_az, np.fmin(kirlilik_level_az, np.fmax(cins_level_kar, cins_level_guc)))
zaman_activation_o = np.fmin(rule13, yikama_zaman_o)
deterjan_activation_n = np.fmin(rule13, deterjan_miktar_n)
sicaklik_activation_yuk = np.fmin(rule13, su_sicaklik_yuk)
ust_pompa_activation_d = np.fmin(rule13, ust_pompa_d)
alt_pompa_activation_y = np.fmin(rule13, alt_pompa_y)
...
zaman_activation_o = np.fmin(rule14, yikama_zaman_o)
deterjan_activation_n = np.fmin(rule14, deterjan_miktar_n)
sicaklik_activation_yuk = np.fmin(rule14, su_sicaklik_yuk)
ust_pompa_activation_d = np.fmin(rule14, ust_pompa_d)
alt_pompa_activation_y = np.fmin(rule14, alt_pompa_y)
...
zaman_activation_o = np.fmin(rule15, yikama_zaman_o)
deterjan_activation_n = np.fmin(rule15, deterjan_miktar_n)
sicaklik_activation_yuk = np.fmin(rule15, su_sicaklik_yuk)
ust_pompa_activation_o = np.fmin(rule15, ust_pompa_o)
alt_pompa_activation_cy = np.fmin(rule15, alt_pompa_cy)
...
zaman_activation_k = np.fmin(rule16, yikama_zaman_k)
deterjan_activation_a = np.fmin(rule16, deterjan_miktar_a)
sicaklik_activation_nor = np.fmin(rule16, su_sicaklik_nor)
ust_pompa_activation_d = np.fmin(rule16, ust_pompa_d)
alt_pompa_activation_o = np.fmin(rule16, alt_pompa_o)
...
deterjan_defuzz = fuzz.defuzz(deterjan_miktar, deterjan_aggregated, 'centroid')
...
def input_plot() :
    ...
    ax0.plot(bulasik_miktar, bulasik_miktar_az,  'b', linewidth = 1.5, label = "az")
    ax0.plot(bulasik_miktar, bulasik_miktar_ort, 'g', linewidth = 1.5, label = "orta")
    ax0.plot(bulasik_miktar, bulasik_miktar_cok, 'r', linewidth = 1.5, label = "çok")
    ax0.set_title("Bulaşık Miktarı")
    ax0.legend()

    ax1.plot(kirlilik_derece, kirlilik_derece_az,  'b', linewidth = 1.5, label = "Az Kirli")
    ax1.plot(kirlilik_derece, kirlilik_derece_ort, 'g', linewidth = 1.5, label = "Orta Kirli")
    ax1.plot(kirlilik_derece, kirlilik_derece_cok, 'r', linewidth = 1.5, label = "Çok Kirli")
    ax1.set_title("Kirlilik derecesi")
    ax1.legend()

    ax2.plot(bulasik_cins, bulasik_cins_has, 'b', linewidth= 1.5, label = "Hassas")
    ax2.plot(bulasik_cins, bulasik_cins_kar, 'g', linewidth= 1.5, label = "Karma")
    ax2.plot(bulasik_cins, bulasik_cins_guc, 'r', linewidth= 1.5, label = "Güçlü")
...
def show_result_plot():
    ...
    ax0.plot(yikama_zaman, yikama_zaman_ck, 'b', linewidth=0.5, linestyle='--', )
    ax0.plot(yikama_zaman, yikama_zaman_k, 'b', linewidth=0.5, linestyle='--', )
    ax0.plot(yikama_zaman, yikama_zaman_o,  'g', linewidth=0.5, linestyle='--')
    ax0.plot(yikama_zaman, yikama_zaman_u, 'r', linewidth=0.5, linestyle='--')
    ax0.plot(yikama_zaman, yikama_zaman_cu, 'r', linewidth=0.5, linestyle='--')
